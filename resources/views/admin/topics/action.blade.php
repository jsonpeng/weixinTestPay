...
@section('content')
    <section class="content-header">
        <h1 class="pull-left">@if(array_key_exists('is_delete', $input)) @if($input['is_delete']) 回收站  @else 批量删除题目 @endif @endif</h1>
            ...
            @if(array_key_exists('is_delete', $input)) 
                @if($input['is_delete'])
                     <div class="pull-left" style="margin-top:15px;">
                         <input type="button" class="btn btn-primary"  value="批量恢复" id="rec">
                     </div>
                @else
                    <div class="pull-left" style="margin-top:15px;">
...
$('#rec,#delete').click(function(){
    ...
    if(action){
        javascript:window.parent.call_back_by_topic_action(action);
    }
});
